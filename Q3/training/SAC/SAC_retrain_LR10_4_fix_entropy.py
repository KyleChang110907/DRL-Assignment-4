...
MODEL_DIR_after      = "./Q3/checkpoints/SAC/LR_1e-4_EP10000_scratch_300kB_fixEntropy"

os.makedirs(MODEL_DIR_after, exist_ok=True)
if not MODEL_DIR_before:
    print(("Train from scratch, no previous model to load."))
elif not os.path.exists(MODEL_DIR_before):
    raise FileNotFoundError(f"Model directory {MODEL_DIR_before} does not exist.")
random.seed(SEED)
np.random.seed(SEED)
...
class Agent(object):
    ...
    def train_step(self):
        ...
        # ★ 建議立刻 clamp，放在這裡 ★
        with torch.no_grad():
            self.log_alpha.clamp_(min=-10.0, max=2.0)   # α ∈ [4e‑5, 7.4]
...
if __name__ == "__main__":
    ...
    # load previously saved actor if it exists
    if MODEL_DIR_before is None:
        print("Train from scratch, no previous model to load.")
    elif not os.path.exists(MODEL_DIR_before):
        raise FileNotFoundError(f"Model directory {MODEL_DIR_before} does not exist.")
    else:
        print(f"Loading existing actor from {MODEL_DIR_before}")

        actor_path = os.path.join(MODEL_DIR_before, "best_actor.pth")
        if os.path.exists(actor_path):
            ckpt = torch.load(actor_path, map_location=agent.device)
            agent.policy.load_state_dict(ckpt)
            print(f"Loaded existing actor from {actor_path}")
    ...
    for ep in range(1, EPISODES+1):
        ...
        print(f"EP {ep:3d} | Return: {ep_ret:.2f} | Time: {time.time()-start_t:.1f}s")
        ...
        if ep % EVAL_INTERVAL == 0:
            ...
            for _ in range(EVAL_EPISODES):
                ...
                ret_e, done_e, tr_e = 0.0, False, False
            ...
            # save every component to separate files (no overwrite)
            prefix = os.path.join(MODEL_DIR_after, f"ep{ep}")
            torch.save(agent.policy.state_dict(),    f"{prefix}_policy.pth")
            torch.save(agent.Q1.state_dict(),        f"{prefix}_Q1.pth")
            torch.save(agent.Q2.state_dict(),        f"{prefix}_Q2.pth")
            torch.save(agent.Q1_tgt.state_dict(),    f"{prefix}_Q1tgt.pth")
            torch.save(agent.Q2_tgt.state_dict(),    f"{prefix}_Q2tgt.pth")
            torch.save(agent.pi_opt.state_dict(),    f"{prefix}_piopt.pth")
            torch.save(agent.q1_opt.state_dict(),    f"{prefix}_q1opt.pth")
            torch.save(agent.q2_opt.state_dict(),    f"{prefix}_q2opt.pth")
            torch.save(agent.alpha_opt.state_dict(), f"{prefix}_alphaopt.pth")
            # save log_alpha
            torch.save(agent.log_alpha,              f"{prefix}_logalpha.pth")
            # save replay buffer
            with open(f"{prefix}_replay.pkl", "wb") as f:
                pickle.dump(agent.replay.buffer, f)

            # update best actor if improved
            if score > best_score:
                best_score = score
                best_path = os.path.join(MODEL_DIR_after, f"best_actor.pth")
                torch.save(agent.policy.state_dict(), best_path)
                print(f"New best actor saved to {best_path}")
            ...
            plt.grid(True)
            plt.tight_layout()
            plt.savefig(MODEL_DIR_after+"/learning_curves.png")
            plt.close()
